...
`config/tieout.yaml` begins as `UNAPPROVED_TEMPLATE`. The owning requirements process must define and obtain human approval for keys, metrics, fixtures, expected outputs, tolerances, and proof boundaries. An implementation, validator, repair, reviewer, or example must never invent or relax those values to obtain `PASS`.
...
8. Do not claim semantic parity, selected-runtime proof, oracle lock, ROI, release, or completion without corresponding approved evidence.
9. State proof boundaries explicitly; local evidence does not automatically prove full-population or distributed behavior.
