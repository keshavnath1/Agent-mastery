...
`config/tieout.yaml` begins as `UNAPPROVED_TEMPLATE`. The owning requirements process must define and obtain human approval for keys, metrics, fixtures, immutable SAS oracle path/hash, generated-output producer, tolerances, privacy limits, and proof boundaries. `test` must create a schema-valid governed fixture manifest and present the completed contract for approval. `validate` must execute the generic keyed tie-out runner whenever local parity is required; unit tests or file existence cannot substitute for `tieout_result.json` and `evidence.json`. An implementation, validator, repair, reviewer, or example must never invent or relax these values to obtain `PASS`.
...
8. A local parity claim requires an approved `config/tieout.yaml`, locked oracle and fixture-manifest hashes, and a schema-valid PASS from `scripts/run_tieout.py`.
9. Do not claim semantic parity, selected-runtime proof, oracle lock, ROI, release, or completion without corresponding approved evidence.
10. State proof boundaries explicitly; local evidence does not automatically prove full-population or distributed behavior.
